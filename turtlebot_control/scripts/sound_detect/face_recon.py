...
def web_cam():
    ...
    while True:
        ...
        # /camera/rgb/image_raw에서 이미지 수신
        data = rospy.wait_for_message("/camera/rgb/image_raw", Image)
        orig_image = Bridge.imgmsg_to_cv2(data, "bgr8")
        orig_image = cv2.cvtColor(orig_image, cv2.COLOR_BGR2RGB)
        orig_image = cv2.resize(orig_image, (320, 240))
        image_mean = np.array([127, 127, 127])
        normalized_image = (orig_image - image_mean) / 128
        transposed_image = np.transpose(normalized_image, [2, 0, 1])
        input_image = np.expand_dims(transposed_image, axis=0).astype(np.float32)
        time_time = time.time()
