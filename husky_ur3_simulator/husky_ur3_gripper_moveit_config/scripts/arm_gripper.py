...
def move_to_front_object(x, y, z):
    ...
    pose_stamped.header.frame_id = 'odom'
    pose_stamped.header.stamp = rospy.Time.now()
    pose_stamped.pose.position.x = x - 0.2  # X축으로 -0.2m 이동
    pose_stamped.pose.position.y = y
...
class YoloDepthAndWait:
    def __init__(self):
        ...
        self.latest_odom_xyz = None     # 최신 객체의 odom 좌표
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            for qr in qr_codes:
                ...
                odom_pt = self.transform_camera_to_odom(X, Y, Z)  # 카메라 좌표계를 odom 좌표계로 변환
            ...
            # 2) YOLO 탐지
            results = self.model(frame, stream=True)  # YOLO 모델로 객체 검출
            for r in results:
                for box in r.boxes:
            ...
            # 4) 객체가 발견되고 최신 좌표가 존재하면, grasp_object() 호출
            # Enter 키(ASCII=13)가 아니라 객체 발견 시 바로 grasp 수행
        ...
        def transform_camera_to_odom(self, X, Y, Z):
            """
            camera_link 좌표계의 (X, Y, Z)를 odom 좌표계로 변환하는 함수
            :param X: 카메라 좌표계 X
            :param Y: 카메라 좌표계 Y
            :param Z: 카메라 좌표계 Z
            :return: odom 좌표계의 (x, y, z) 또는 None
            """
            global tf_buffer
            try:
                ...
                pt_odom = mat @ pt_cam  # 변환 행렬을 사용하여 odom 좌표계로 변환
                return pt_odom[:3]  # (x, y, z) 반환
            except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        move_group = MoveGroupCommander("ur3_manipulator")  # MoveGroupCommander 초기화 (그룹 이름: ur3_manipulator)
        ...
        action_executor.grasp_object()  # 객체를 잡는 시퀀스 실행
